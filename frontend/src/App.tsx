...
import { checkHealth, fetchQuestions, runCode, runSampleTests, submitQuestion } from './api';
...
const EXAM_DURATION_SECONDS = 27 * 60 + 10; // 00:27:10, per spec example
...
export default function App() {
  ...
  // Load question bank from the backend once.
  useEffect(() => {
    fetchQuestions()
      .then((qs) => {
        setQuestions(qs);
  ...
  const secondsLeft = useCountdown(EXAM_DURATION_SECONDS, handleExpire, testStarted);
  ...
  if (loadError) {
    return (
      <div className="fatal-error-screen">
        ...
        <p>{loadError}</p>
        <p className="fatal-error-hint">Make sure the backend is running on http://localhost:4000.</p>
      </div>
    );
  }
